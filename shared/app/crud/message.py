...
def count_search_users(db: Session, query: str):
    ...
    return (
        ...
        .filter(
            User.display_name.ilike(f"%{query}%") | User.user_id.ilike(f"%{query}%")
        )
        .scalar()
    )
...
def create_message(
    db: Session, event_id: str, room_id: str, sender_id: str, content: str
):
    db_message = Message(
        event_id=event_id, room_id=room_id, sender_id=sender_id, content=content
    )
    db.add(db_message)
    db.commit()
...
def get_message_stats(db: Session, days: int = 7):
    ...
    daily_stats = (
        ...
        .order_by(func.date_trunc("day", Message.timestamp))
        .all()
    )
...
def get_hourly_activity(db: Session, days: int = 7):
    ...
    return (
        ...
        .order_by(extract("hour", Message.timestamp))
        .all()
    )
...
def get_user_interaction_pairs(
    db: Session, days: int = 7, min_count: int = 3, room_id: Optional[str] = None
) -> List[Tuple]:
    """
    获取用户互动对，支持按房间筛选
    """
...
def get_message_trends(db: Session, days: int = 7, interval: str = "day"):
    ...
    return (
        db.query(group_by.label("period"), func.count(Message.event_id).label("count"))
        .filter(Message.timestamp >= start_date)
        .group_by("period")
        .order_by("period")
        .all()
...
def get_conversation_patterns(db: Session, days: int = 7):
    ...
    # 获取消息序列
    messages = (
        db.query(Message.room_id, Message.sender_id, Message.timestamp, Message.content)
        .filter(Message.timestamp >= start_date)
        .order_by(Message.room_id, Message.timestamp)
        .all()
    )
...
def get_activity_heatmap(
    db: Session, days: int = 7, room_id: Optional[str] = None
) -> List[Tuple]:
    """
    获取活动热力图数据
    ...
    return [
        tuple(row)
        for row in query.group_by("weekday", "hour").order_by("weekday", "hour").all()
    ]


def get_user_hourly_activity(
    db: Session, days: int = 7, room_id: Optional[str] = None, limit: int = 50
) -> List[Tuple]:
    """
    获取基于用户的每小时活动数据
    ...
    top_users_query = db.query(
        Message.sender_id, func.count(Message.event_id).label("total_messages")
    ).filter(Message.timestamp.between(start_date, end_date))
    ...
    top_users = (
        top_users_query.group_by(Message.sender_id)
        .order_by(func.count(Message.event_id).desc())
        .limit(limit)
        .all()
    )
    ...
    query = db.query(
        ...
        Message.timestamp.between(start_date, end_date), Message.sender_id.in_(user_ids)
    )
    ...
    results = (
        query.group_by(Message.sender_id, func.extract("hour", Message.timestamp))
        .order_by(Message.sender_id, func.extract("hour", Message.timestamp))
        .all()
    )
