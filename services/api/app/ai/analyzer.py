            ...
            system_prompt = """你是一个专门进行数据分析的AI助手。请按照以下格式返回JSON：
{
    ...
    async def generate_summary(self, messages: List[Dict]) -> dict:
        ...
        try:
            texts = [
                f"{msg['sender']['display_name']}: {msg['content']}" for msg in messages
            ]
            combined_text = "\n".join(texts)
    ...
    def generate_user_interaction_chart(self, messages: List[Dict]) -> go.Figure:
        ...
        # 创建饼图
        fig = go.Figure(
            data=[go.Pie(labels=user_counts.index, values=user_counts.values, hole=0.3)]
        )

        fig.update_layout(title="用户参与度分布", template="plotly_dark")
    ...
    def analyze_user_interactions(self, messages: List[Dict]) -> nx.Graph:
        ...
        for i in range(len(messages) - 1):
            ...
            # 检查消息是否在同一房间
            if (
                current["room_id"] == next_msg["room_id"]
                and current["sender_id"] != next_msg["sender_id"]
            ):
                # 添加或更新边的权重
                weight = G.get_edge_data(
                    current["sender_id"], next_msg["sender_id"], {"weight": 0}
                )["weight"]

                G.add_edge(
                    current["sender_id"], next_msg["sender_id"], weight=weight + 1
                )
